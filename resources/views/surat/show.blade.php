...
@section('title', 'Detail Surat - Arsip Surat Ngodingyuk')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
        <!-- Header -->
        <div class="bg-gradient-to-r {{ $surat->jenis == 'masuk' ? 'from-green-600 to-emerald-600' : 'from-orange-600 to-amber-600' }} px-8 py-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <div class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-lg">
                        <i class="fas fa-{{ $surat->jenis == 'masuk' ? 'inbox' : 'paper-plane' }} text-2xl {{ $surat->jenis == 'masuk' ? 'text-green-600' : 'text-orange-600' }}"></i>
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-white">
                            {{ $surat->jenis == 'masuk' ? 'Surat Masuk' : 'Surat Keluar' }}
                        </h1>
                        <p class="text-white/90 mt-1">{{ $surat->nomor_surat }}</p>
                    </div>
                </div>
                    ...
                    <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 border border-indigo-100">
                        ...
                        <div class="space-y-4">
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $surat->nomor_surat }}</p>
                            </div>
                            
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Nomor Agenda</p>
                                <p class="text-lg font-bold text-gray-900">{{ $surat->nomor_agenda }}</p>
                            </div>
                            
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Perihal</p>
                                <p class="text-base font-semibold text-gray-900">{{ $surat->perihal }}</p>
                            </div>
                            
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Tanggal</p>
                                <p class="text-lg font-bold text-gray-900">
                                    <i class="fas fa-calendar mr-2 text-indigo-600"></i>{{ $surat->tanggal->format('d F Y') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-100">
                        ...
                        <div class="space-y-4">
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $surat->pengirim }}</p>
                            </div>
                            ...
                            <div class="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-600 mt-1">{{ basename($surat->file_path) }}</p>
                                    </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-100">
                        ...
                        @if($surat->keterangan)
                            <div class="bg-white rounded-xl p-5 border border-gray-100">
                                <p class="text-gray-900 leading-relaxed">{{ $surat->keterangan }}</p>
                            </div>
                        @else
                        ...
                        <div class="space-y-3">
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                ...
                                <p class="text-sm font-semibold text-gray-900">
                                    <i class="fas fa-calendar-plus mr-2 text-indigo-600"></i>{{ $surat->created_at->format('d F Y, H:i') }}
                                </p>
                                <p class="text-xs text-gray-500 mt-1">{{ $surat->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="bg-white rounded-xl p-4 border border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Terakhir diperbarui</p>
                                <p class="text-sm font-semibold text-gray-900">
                                    <i class="fas fa-sync-alt mr-2 text-green-600"></i>{{ $surat->updated_at->format('d F Y, H:i') }}
                                </p>
                                <p class="text-xs text-gray-500 mt-1">{{ $surat->updated_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
